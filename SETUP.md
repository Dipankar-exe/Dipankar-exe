...
```powershell
python scripts\dotify.py assets\my_photo.png -o assets\portrait --cols 100 --equalize --detail 0.5 --color --reveal
```
...
python scripts\dotify.py assets\my_photo.png -o assets\portrait --cols 88 --equalize --detail 0.5 --animate

# literal 0s and 1s instead of dots
python scripts\dotify.py assets\my_photo.png -o assets\portrait --mode binary --cols 62 --equalize --detail 0.5

# plain text art — paste the .txt into a ``` code block in the README
python scripts\dotify.py assets\my_photo.png -o assets/portrait --mode ascii --cols 80
python scripts\dotify.py assets\my_photo.png -o assets/portrait --mode braille --cols 100
```
...
- `--square` crops to 1:1, with `--focus X,Y` to say which point should end up centred
  (`0.55,0.45` for a face sitting right of and above the middle). `my_photo.png` was
...
background. `my_photo.png` already has one, which is why the orange backdrop vanishes.
...
```powershell
python scripts\cards.py --user <YOUR_GITHUB_USERNAME> --out assets
```
